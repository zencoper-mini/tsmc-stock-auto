...
def update_database():
    ...
    cursor = conn.cursor()
    # 建立資料表，將「成交張數」改為「成交股數」
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS stock_data_v2 (
            日期 TEXT, 成交股數 REAL, 成交金額 TEXT, 收盤價 TEXT, 法人項目 TEXT, 買賣超股數 REAL,
            PRIMARY KEY (日期, 法人項目)
        )
    ''')
    conn.commit()
                ...
                if inst_data and inst_data.get("stat") == "OK":
                    ...
                    if not tsmc_row.empty:
                        ...
                        if not day_stock.empty:
                            ...
                            # 直接使用原始成交股數，不除以 1000
                            raw_volume = clean_val(day_stock['成交股數'].values[0])
                            ...
                            res = [
                                {'日期': target_date, '成交股數': raw_volume, '成交金額': day_stock['成交金額'].values[0], '收盤價': day_stock['收盤價'].values[0], '法人項目': '外資', '買賣超股數': clean_val(tsmc_row[idx_foreign].values[0])},
                                {'日期': target_date, '成交股數': raw_volume, '成交金額': day_stock['成交金額'].values[0], '收盤價': day_stock['收盤價'].values[0], '法人項目': '投信', '買賣超股數': clean_val(tsmc_row[idx_trust].values[0])},
                                {'日期': target_date, '成交股數': raw_volume, '成交金額': day_stock['成交金額'].values[0], '收盤價': day_stock['收盤價'].values[0], '法人項目': '自營商', '買賣超股數': dealer_sum}
                            ]
